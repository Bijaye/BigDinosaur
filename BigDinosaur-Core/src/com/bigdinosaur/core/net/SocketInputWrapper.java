...
public class SocketInputWrapper extends FilterInputStream {
  ...
  SocketInputWrapper(Socket s, InputStream is) {
    ...
    if (hasChannel) {
      Preconditions.checkArgument(is instanceof SocketInputWrapper,"Expected a SocketInputStream when there is a channel. " +
          "Got: %s", is);
    }
  }
  ...
  public void setTimeout(long timeoutMs) throws SocketException {
    if (hasChannel) {
      ((SocketInputWrapper) in).setTimeout(timeoutMs);
    } else {
