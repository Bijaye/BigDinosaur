...
public class BdDns {
  ...
  private static final Log LOG = LogFactory.getLog(BdDns.class);
   ...
   * @param ns The host name of a reachable BdDns server
   * @return The host name associated with the provided IP
   * @throws NamingException If a NamingException is encountered
   */
  public static String reverseBdDns(InetAddress hostIp, String ns)
    ...
    try {
      attribute = ictx.getAttributes("BdDns://"               // Use "BdDns:///" if the default
                         + ((ns == null) ? "" : ns) +
  ...
  /**
   * Like {@link BdDns#getIPs(String, boolean), but returns all
  ...
  public static String[] getHosts(String strInterface, String nameserver)
    ...
    for (int ctr = 0; ctr < ips.length; ctr++) {
      try {
        hosts.add(reverseBdDns(InetAddress.getByName(ips[ctr]),
                             nameserver));
